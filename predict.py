...
if not is_land:
    if object_name:
        ...
        def name_similar(name_val):
            if pd.isna(name_val): return False
            return sum(1 for kw in keywords if kw in str(name_val).lower()) >= min(2, len(keywords))
        ...
        if len(name_filtered) >= 3:
            ...
            if len(name_filtered) >= 3: similar = name_filtered
    ...
    if wall_material:
        ...
        if len(material_filtered) >= 3: similar = material_filtered

# Город
if city and len(similar) >= 5:
    city_filtered = similar[similar['address'].str.contains(city, na=False)]
    if len(city_filtered) >= 3: similar = city_filtered
...
# ============================================================
# Поиск 5 ближайших с усиленными весами
# ============================================================
...
if is_land:
    ...
    features_scaled = scaler.fit_transform(similar[feature_cols].fillna(0))
    features_scaled[:, 2] *= 3  # ВРИ ×3
    nn.fit(features_scaled)
    
    use_code = pd.factorize(df['permitted_use'])[0][df['permitted_use'] == permitted_use]
    use_code = use_code[0] if len(use_code) > 0 else 0
    obj_scaled = scaler.transform([[area, build_year, use_code]])
    obj_scaled[:, 2] *= 3
    distances, indices = nn.kneighbors(obj_scaled)
    ...
    features_scaled = scaler.fit_transform(similar[feature_cols].fillna(0))
    features_scaled[:, 3] *= 5  # Наименование ×5
    features_scaled[:, 4] *= 3  # Материал ×3
    nn.fit(features_scaled)
    ...
    obj_scaled = scaler.transform([[area, build_year, type_code, name_code, material_code]])
    obj_scaled[:, 3] *= 5  # Наименование ×5
    obj_scaled[:, 4] *= 3  # Материал ×3
    distances, indices = nn.kneighbors(obj_scaled)
...
for _, analog in analogs.iterrows():
    corr = 1.0
    if area > 100 and analog['area'] < 100: corr *= 0.95
    elif area < 50 and analog['area'] > 50: corr *= 1.05
    year_diff = build_year - analog['build_year']
    if abs(year_diff) > 5: corr *= 1 + (year_diff * 0.005)
    if city and city not in str(analog.get('address', '')): corr *= 0.90
...
if is_land and permitted_use: justification += f"\nВРИ: {permitted_use}"
if not is_land:
    if object_name: justification += f"\nНаименование: {object_name}"
    if wall_material: justification += f"\nМатериал стен: {wall_material}"
justification += f"""
...
if city: justification += f", город={city}"
if is_land and permitted_use: justification += f", ВРИ похожие на «{permitted_use}»"
if not is_land and object_name: justification += f", наименование похожее на «{object_name}»"
if not is_land and wall_material: justification += f", материал «{wall_material}»"
...
for i, (_, a) in enumerate(analogs.iterrows(), 1):
    ...
    if mat: justification += f" | Материал: {mat}"
    if use: justification += f" | ВРИ: {use}"
...
result = {
    "object": {
        "kadastr": clean_val(kadastr, 20), "area": area, "build_year": build_year,
        "object_type": object_type, "permitted_use": clean_val(permitted_use, 50),
        "name": clean_val(object_name, 80), "wall_material": clean_val(wall_material, 30),
        "city": clean_val(city, 30), "address": clean_val(address, 120)
    },
    "predicted": {"price_per_sqm": round(price_sqm), "price_total": round(price_total)},
    "calculation": {"ml_prediction": round(price_sqm), "avg_analogs": round(avg_analog),
                    "weighted_avg": round(weighted_avg_price), "deviation_pct": round(diff_pct, 1)},
    "justification": justification, "analogs": [], "search_features": search_desc
}

for i, (_, a) in enumerate(analogs.iterrows()):
    year = int(a.get('build_year', 0)) if not pd.isna(a.get('build_year')) else 0
    result["analogs"].append({
        "num": i + 1, "kadastr": clean_val(a.get('kadastr', ''), 20),
        "name": clean_val(a.get('name', ''), 80), "area": round(float(a['area']), 1),
        "price_per_sqm": round(float(a['price_per_sqm'])), "price_total": round(float(a.get('price_total', 0))),
        "build_year": year, "object_type": clean_val(a.get('object_type', ''), 30),
        "permitted_use": clean_val(a.get('permitted_use', ''), 50),
        "wall_material": clean_val(a.get('wall_material', ''), 30),
        "address": clean_val(a.get('address', ''), 120), "correction": corrections[i],
